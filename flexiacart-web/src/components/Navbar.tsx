...
import { ShoppingCart, Search, MapPin, Zap, Package, Gift, Menu, X, MessageSquare, HelpCircle, Bell, Store, User, SlidersHorizontal } from 'lucide-react';
    ...
    <header className="sticky top-0 z-40 bg-white border-b border-gray-100 shadow-sm">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20 gap-4">
          ...
          {/* Search Bar with Section 8 Advanced Filtering link */}
          <form onSubmit={handleSearchSubmit} className="hidden md:flex flex-1 max-w-md relative">
            <input
              ...
              className="w-full bg-gray-50 border border-gray-200 rounded-full py-2.5 pl-4 pr-20 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:bg-white transition-all font-semibold"
            />
            <div className="absolute right-2 top-1.5 flex items-center gap-1">
              ...
              <button type="submit" className="p-1.5 text-gray-400 hover:text-orange-500">
                <Search className="w-4 h-4" />
              </button>
            </div>
          </form>

          {/* Nav Icons & Standard Account Menu */}
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            {/* Standard User Account Button (Section 2) */}
            <Link
              href="/login"
              className="flex items-center gap-1.5 bg-gray-100 hover:bg-gray-200 px-3 py-2 rounded-xl text-xs font-bold text-gray-800 transition-colors"
            >
        ...
        <div className="md:hidden bg-white border-t border-gray-100 px-4 pt-2 pb-4 space-y-3 shadow-lg">
          ...
          <div className="space-y-1">
            ...
            <Link
              ...
              className="block px-3 py-2 rounded-lg text-base font-bold text-gray-800 hover:bg-orange-50"
            >
              🔍 Advanced Search & Filters (Section 8)
            </Link>
            <Link
              href="/orders"
              onClick={() => setMobileMenuOpen(false)}
            ...
            <Link
              ...
              className="block px-3 py-2 rounded-lg text-base font-bold text-gray-800 hover:bg-orange-50"
            >
              🏬 Public Seller Storefronts (Section 7)
            </Link>
            <Link
              href="/messages"
              onClick={() => setMobileMenuOpen(false)}
